...
| **权限策略** | **全部 15 张表的 schema `permission` 均为 `read/create/update/delete: false`**，客户端不可直连，所有读写走云函数 | 本业务有发布审核、治理下架、限购、库存原子扣减等规则，若开放客户端直连 JQL，这些规则可被绕过。统一走云函数使业务规则不可旁路 |
...
| `grouporder-oplog` | 统一操作日志，9 字段覆盖 11 类事件 |
| `grouporder-ops-verify` | 敏感权限二次验证 |
...
团长保存商品时自动沉淀的用户私有商品库，用于创建活动时复用历史商品（D-063）。完整实现规格见 `GOODS_LIB_SPEC.md` v0.2。
...
**必记的 11 类事件**：运营账号登录成功/失败/停用/角色变更；二次验证成功/失败/过期；举报领取/结论/结案/复核/结果变更；内容检测查看/复核/处置；活动下架/恢复/失败尝试；发布者警告/临时限制/永久限制/解除；完整姓名电话地址的查看成功与拒绝；账号解绑/重新绑定/拒绝/重复请求；Excel 事件查询；运营统计查询成功/拒绝与统计权限变更；注销删除匿名化事项登记与状态变化。

**硬性约束**（OPS §13）：
- 日志**不得记录**密码、二次验证凭证或可继续使用的下载链接
...
### 10.10 `grouporder-ops-verify` 敏感权限二次验证

| 字段 | bsonType | 必填 | 说明 |
|---|---|---|---|
| `_id` | objectId | - | 系统自动生成 |
| `operator_uid` | string | ✓ | 运营账号 |
| `verify_time` | timestamp | ✓ | 验证时间 |
| `expire_time` | timestamp | ✓ | 有效期截止 |
| `result` | int | ✓ | 1成功 0失败 |
| `scope_case_no` | string | | **授权覆盖的事项编号** |
| `scope_object_ids` | array | | 授权覆盖的对象范围 |
| `invalidated` | int | | 是否已被提前失效 |
| `create_date` | timestamp | ✓ | |

**约束**（OPS §8.2、D-068）：授权**只覆盖当前事项和必要对象**，不能据此浏览其他用户或活动；页面重新打开、会话失效、账号停用或权限撤销后必须**重新校验**，不能沿用旧结果。

- **验证形式**：重新输入本账号的登录密码。本表**不关心验证方式**——只记录验证时间、有效期、结果与授权范围，后续若升级为 TOTP 动态口令，仅需改动云函数的校验逻辑，表结构不动。
- **有效期 1 天**（写入 `expire_time`）。免于重复验证需同时满足：同一事项（`scope_case_no` 一致）、同一授权对象范围（`scope_object_ids` 覆盖）、同一会话。换事项必须重新验证。
- **连续失败 5 次锁定 15 分钟**：失败记录以 `result = 0` 写入本表，云函数按 `operator_uid` 统计近期失败次数判定锁定。锁定期间即使密码正确也返回失败。
- 本表**不得存储密码或任何可还原的凭证**，`result` 只记成败。

### 10.11 敏感资料访问链路

OPS §8 要求的受控解敏，由三张表协作完成，不单独建表：

1. `grouporder-ops-verify` 确认二次验证在有效期内且 `scope` 覆盖本次对象
2. 校验 `uni-id-users.role` 含 `ops-privacy`，且关联事项（举报/申诉/隐私事项）状态有效并已分配给本人
3. 通过后返回该事项必要的完整字段，同时向 `grouporder-oplog` 写入一条 `action_type = view_sensitive` 的记录

查看成功、拒绝、二次验证失败与权限失效**四种结果均须记录**。首版不提供敏感详情批量导出、复制清单或下载 Excel 的能力。
...
| ops-verify | `operator_uid + expire_time` | 有效性校验 |
...
| 编号       | 事项                      | 现状                                                       |
| -------- | ----------------------- | -------------------------------------------------------- |
| ~~A-01~~ | 商品封面图是否必填               | **已裁定：必填**（D-041），PRD §5.3 已同步，与本文档表定义一致                 |
| ~~A-02~~ | 活动详情是否向参与者展示「有效总份数」 | **已裁定：展示**（D-016 更新），活动详情向所有访问者展示活动有效总份数 |
| ~~A-03~~ | 运营后台范围                  | **已裁定：按 OPS 完整规格实施**（D-042），不采纳评审 H-05 的削减建议；第二批表按完整规格设计 |
| A-04     | 微信服务类目与主体核验（P1-09、B-01） | 未核验，阻塞 schema 落地                                         |
| ~~A-05~~ | Excel 文件与下载链接有效期 | **已裁定**（D-071）：临时地址 30 分钟、文件保留 60 天、每次下载重新鉴权、版本失效随保留期清除。P1-07 中举报/审计记录期限与备份轮换仍待合规确认 |
| ~~A-07~~ | 两工程 uni-id-pages 版本对齐 | **已完成**（2026-09-15）：由产品负责人在 HBuilderX 插件市场将 admin 升级至 1.1.28，两侧一致；`status = 4` 缺失已由项目目录的覆盖版补回（详见 §3） |
| ~~A-06~~ | 图片异步检测与审核放行的边界 | **已裁定：不等待回调**（D-058）。文字检测或人工审核通过即放行，图片回调命中后转治理下架并通知团长，不回退业务状态 |
...
| 2026-09-15 | v1.6 | 按 `NAV_AND_GOODS_ALIGNMENT.md` 落实导航与交付方式方案：新增 `grouporder-todo-dismiss` 表（D-062 待办派生视图的唯一落库状态）；`activity` 新增 `delivery_type`（D-060，发布后不可修改）；`order.consignee_address` 由必填改条件必填（自提活动不采集地址）；表清单第一批增至 8 张、合计 18 张 |
