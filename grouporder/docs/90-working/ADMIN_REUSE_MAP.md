...
- 事实来源：`grouporder-admin` 工程实际代码（已逐文件核对）、`OPS_ADMIN_REQUIREMENTS.md`、`DECISIONS.md`
...
### 6.2 17 个权限点（`uni-id-permissions.init_data.json`）
...
| `ops-super` | 超级管理员 | **全部 17 个 `ops-*` 权限点**（D-076）：workbench、content-review、content-report、content-activity、search、stat-view、privacy-appeal、privacy-case、audit-export、audit-oplog、audit-login、sys-account、sys-role、sys-permission、sys-menu、sys-config、sys-error |
| `ops-content` | 内容运营 | workbench、content-review、content-report、content-activity、search、audit-export |
| `ops-privacy` | 账号与隐私专员 | workbench、search、privacy-appeal、privacy-case、audit-export |
| `ops-auditor` | 审计查看者 | workbench、audit-export、audit-oplog、audit-login |

**`ops-super` 持有全部业务权限点**（D-076，2026-09-18 改）——原先「超管不自动继承业务权限」的隔离设计已被推翻，理由与 D-072 同源：单人或极小团队里「能授权的人」与「能看数据的人」本就是同一个人，形式化隔离只产生摩擦。约束仍由全量审计承担。

> 注意两件**没有**跟着放开的事：① **修改或删除审计日志**对所有角色关闭，超管同样没有（OPS §13）；② `admin` 内置账号仍应在初始化后停用（§8），原因不再是权限隔离，而是它由 `registerAdmin` 写死角色、不受权限表达式约束。

各角色实际可见的叶子菜单（已机械校验）：

| 角色 | 可见菜单 |
|---|---|
| 超级管理员 | **全部菜单**：工作台 / 活动发布审核 / 举报与内容审核 / 活动与商品 / 全局检索 / 统计总览 / 团长与活动 / 账号绑定申诉 / 隐私与注销事项 / Excel 事件 / 操作日志 / 登录日志 / 运营账号 / 角色管理 / 权限管理 / 菜单管理 / 平台配置 |
| 内容运营 | 工作台 / 活动发布审核 / 举报与内容审核 / 活动与商品 / 全局检索 / Excel 事件 |
| 账号与隐私专员 | 工作台 / 全局检索 / 账号绑定申诉 / 隐私与注销事项 / Excel 事件 |
| 审计查看者 | 工作台 / Excel 事件 / 操作日志 / 登录日志 |

> **菜单只能控制入口，控制不了数据粒度。** 不过 D-072 之后，OPS §3.2 里「仅查看关联活动摘要」「仅限处理事项」这类**按事项收窄的数据粒度限定已经取消**，可见范围就等于页面访问权限；页面内仍需按角色判断的只剩「审计日志按 `ops-audit-oplog`」与「统计按 `ops-stat-view`」两处，且 **D-076 起超管两者都持有**。

### 6.4 菜单结构（`opendb-admin-menus.init_data.json`，共 59 条 / 启用 22 条）
...
**未完成上述四步就开菜单，运营只会看到一个永远空白的页面。** 该能力属 OPS 四角色之外的技术运维职责，**归属已确认由超级管理员持有**（产品负责人 2026-09-17）；D-076 之后超管本就全权，这一条不再需要单独论证。
...
**`admin` 不受任何 permission 控制，自动拥有全部菜单与接口权限**，而 `registerAdmin` 云函数写死 `role: ['admin']`。D-076 之后问题不在权限大小（`ops-super` 同样全权），而在 `admin` **跳过整个权限查询、不受权限表达式与菜单模型约束**，其操作也无法按角色归因——等于留一个绕过审计体系的后门。
...
**不执行第 5 步，OPS §3.1 的隔离就不成立**——admin 账号能看到全部菜单，包括系统管理与全部审计。
...
| 2026-09-18 | v0.4 | 按 **D-073/074/075** 与 D-072 残留清理：§7 权限模型删除 `ops-privacy-sensitive` 权限点与 `go_sensitive`「敏感资料查看」菜单项（工程 `uni-id-permissions` / `opendb-admin-menus` / `uni-id-roles` 三份初始数据已同步删除）；可见菜单表与初始化第 5 步的说明同步；「按事项收窄数据粒度」的注记按 D-072 改写 |
| 2026-09-20 | v0.7 | 内置权限点 14 → **15**：补出 `READ_UNI_ID_LOG` 并授予 `ops-super` 与 `ops-auditor`。`uni-id-log.schema.json` 的读权限由 `'READ_UNI_ID_LOG' in auth.permission` 校验，ADM-39 当初补 14 个内置权限点时遗漏了它，不补则 A-14 登录日志 tab 恒为空列表 |
| 2026-09-20 | v0.6 | 更正 §6.2 标题的「18 个权限点」——该表实列 17 行，`uni-id-permissions.init_data.json` 实有 17 条 `ops-*`，§6.3 与 v0.5 变更记录也都写的 17，只有标题是旧数 |
| 2026-09-18 | v0.5 | 按 **D-076** 改写权限模型：`ops-super` 由 7 个系统权限点扩为**全部 17 个 `ops-*` 权限点**，可见菜单表改为「全部菜单」，§7 的「不含任何业务权限点」说明整段重写；菜单总数 60 → 59（`go_sensitive` 已删）。两项未跟着放开的例外（审计日志不可改删、`admin` 账号仍需停用）就地标注 |
