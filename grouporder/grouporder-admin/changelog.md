## 3.0.0（2026-07-29）
内置页面（除uni_modules依赖外）均升级为 vue3 组合式语法，并从3.0.0版本起，不再兼容 vue2 ，仅支持 vue3 运行
## 2.6.2（2026-07-15）
- 新增 升级中心发布 wgt 更新时，静默更新和强制更新功能现在是互斥了（之前静默更新优先）
- 新增 升级中心已上线的更新可更改为下线
- 修复 uni统计首页 今日/昨日数据缺失时页面报错的问题
## 2.6.1（2026-06-30）
- 修复 前端错误统计会显示app崩溃统计的问题
- 优化 使用 prettier 统一格式化代码
## 2.6.0（2026-06-01）
- 新增 内嵌模式，支持内嵌到其他项目中当子页面 [详情](https://doc.dcloud.net.cn/uniCloud/admin.html#embed)
## 2.5.16（2026-05-28）
- 新增 支持解析 UTSError
## 2.5.15（2026-05-25）
- 修复 使用除阿里云服务空间外时，在升级中心上传文件时返回链接错误
- 新增 升级中心上传文件时默认上传至 /upgrade-center 目录
## 2.5.14（2026-05-25）
- 修复 HarmonyOS 6.0.0 设备访问发布页会被识别为 Android 设备的问题
- 修复 部分宽屏设备访问发布页时无下载按钮
- 修复 sourcemap 解析错误时，有缓存时却重新解析
- 修复 当使用除阿里云服务空间时，在编辑应用详情中上传文件时返回链接错误
- 更新 uni-data-checkbox 插件
## 2.5.13（2025-11-21）
