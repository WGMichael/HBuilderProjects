export default {
  ...
  /*
	...
	*/
  isAdmin: true, // 区分管理端与用户端。admin 工程必须为 true，模块升级后请检查此项
  loginTypes: [
    // "qq",
    // "xiaomi",
    // "sinaweibo",
    ...
    // ⚠ 运营后台仅启用用户名密码登录（D-069）。
    //   运营账号由超级管理员创建、不对外开放注册，微信/苹果/华为登录在后台场景没有意义；
    //   smsCode 需付费开通短信服务且要采集手机号，与 D-029 冲突。
    //   且二次验证形式为「重新输入登录密码」（D-068），前提是账号必须有密码。
    //   模块升级后此处会被默认值覆盖，请务必重新核对。
    'username',
  ],
  // 政策协议
  agreements: {
